...
* `width [us]` : 시뮬레이션용 ZC 펄스의 너비를 설정 (측정값에는 영향 없음) (예: `width 5`)
* `track on` / `track off` : 자동 주파수 트래킹(PI 제어기) 활성화/비활성화
* `target [us]` : 목표(Target) ZC 딜레이 설정 (현재 측정값보다 높으면 Freq UP, 낮으면 DOWN) (예: `target 5.0`)
* `kp [값]` / `ki [값]` : PI 제어기의 비례(Kp) 및 적분(Ki) 게인 실시간 설정 (빠른 추적을 위해 `ki 50` 등 높은 값 권장)

### 4.2. 트래킹 테스트(PLL Lock) 시나리오
1. **하드웨어 연결:** `GPIO 4` (LEAD 출력)는 IOMUX 내부 루프백이 적용되어 물리적 점퍼선 연결이 필요 없습니다. `GPIO 10` (가상 ZC 펄스 출력)과 `GPIO 9` (ZC 캡처)를 점퍼선으로 연결합니다.
2. 터미널에서 `on`을 입력하여 PWM을 20kHz로 활성화합니다.
3. `track on`을 입력하여 트래킹 제어 루프를 시작합니다.
4. `target [값]` 명령어를 통해 목표 딜레이를 변경하며 알고리즘의 동작을 시뮬레이션합니다.
5. 터미널 로그(`[Auto-Track]`)를 통해 측정된 딜레이(Meas Delay)가 목표치(Target) 방향으로 제어되어, 오차 범위 내에 들어오는 순간 주파수 증감이 정지하며(Lock) 완벽하게 추적되는 것을 확인합니다.
