...
4. **[완료]** 시뮬레이션 태스크 실행을 통한 Capture 측정값(위상차) 검증 및 PI 제어기를 통한 공진 주파수 자동 추적(Auto-Tracking) 루프 완성.
   ...
   * **[v1.2.0 신규]** ISR 내부의 실수(float) 연산 제외 및 하드웨어 프리스케일러(prescaler)를 이용한 1/100 분주 적용으로 CPU 부하 최소화(1% 미만) 달성.
5. **[완료]** 시작 펄스 추출 회로 개선. 
   * 물리적인 점퍼선 없이 `PIN_INPUT_ENABLE` 매크로를 이용해 IOMUX 내부 루프백을 구현하여 캡처하도록 변경 완료.
6. **[진행 대기 - 다음 작업]** 실제 하드웨어 연결 및 HIL 기반 검증 로직을 실전 오존셀 구동 코드로 이식.
